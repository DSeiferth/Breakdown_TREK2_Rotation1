...
class ModelBetaFit:
    def __init__(self, T=1, samplingStep=2e-7, order=4, cutoff_fs=5e3, ResamplingFreq=200e3, expData=None):
        self.__T = T
    ...
    def GenerateGillespieTrajectory(self):
        ...
        [prob, sample_states] = Gillespie.simulation_traj(matrix=self.__A, startState=0, T=self.__T, sampling_step=self.__samplingStep)
    ...
    def Filter(self, plot=0, name=None):
        ...
        if plot:
            subset = int(0.01 * 1/self.__samplingStep )
            f_size =  14
            fig, ax = plt.subplots()
    ...
    def WriteOutTrajectories(self, outFolder):
        ...
        np.savetxt(outFolder + str(int(self.__cutoff_fs/1e3)) + 'kHz_'+str(self.__mV)+'_mV_noNoise.txt', self.__y_sos[0::downsampling])
        np.savetxt(outFolder + str(int(self.__cutoff_fs/1e3)) + 'kHz_'+str(self.__mV)+'_mV_Noise.txt', self.__y_sos_noise[0::downsampling])
        ...
        np.savetxt(outFolder + str(int(self.__cutoff_fs/1e3)) + 'kHz_'+str(self.__mV)+'_mV_NoFilter_BaselineNoise.txt', traj)
