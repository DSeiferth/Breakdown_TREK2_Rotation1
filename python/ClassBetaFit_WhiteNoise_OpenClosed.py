...
class ModelBetaFit:
    ...
    def FIT_param(self, A, states, currentLevels, baselineNoise):
        ...
        self.__addNoise = baselineNoise[0] # 1st is std dev for closed, 2nd for open level
    ...
    def GenerateGillespieTrajectory(self):
        ...
        noise_closed = np.random.normal(0, self.__addNoise[0],N) * closed_vec
        noise_open = np.random.normal(0, self.__addNoise[0],N) * open_vec
        print('std. dev. of noise (closed, open) before filtering', np.std(noise_closed), np.std(noise_open))
        self.__signal_noise = sample_indistinguishable + noise_closed + noise_open
        print()
    ...
    def PlotAmplitudeDistribution(self, log=1, limits = [[-75, 5], [1e-7, 1e0]],legend=0, exp=0, name=None, f_size=14):
        ...
        ax.plot(self.__binEdges_noNoiseFilter[:-1], self.__hist_NoiseFilter, '--',  label='noise (added pre filter): $\sigma$ = ' +str(self.__addNoise), linewidth=3)
