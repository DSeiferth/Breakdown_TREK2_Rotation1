...
class ModelBetaFit:
    def __init__(self, T=1, samplingStep=2e-7, order=4, cutoff_fs=5e3, ResamplingFreq=200e3, 
                 expData=None, binning=100, customNoise=0, noiseFac=10, customNoise_Open=0):
        self.__T = T
        ...
        self.__customNoise = customNoise
        self.__noiseFac = noiseFac
        self.__customNoise_Open = customNoise_Open
    ...
    def GenerateGillespieTrajectory(self):
        ...
        prob, sample_states, NoGillespie, NoMissed = Gillespie.simulation_traj(matrix=self.__A, startState=0, 
                                                                               T=self.__T, 
                                                                               sampling_step=self.__samplingStep)
        ...
        # vector that is zero when closed and one when open
        open_vec = sample_states.copy()
        for i in self.__closed:
            index = sample_states == i
            open_vec[index] = 0
        index = sample_states == -1
        ...
        for i in range(len(self.__openLevel)):
            index = sample_states == self.__open[i]
            open_vec[index] = 1
        self.__open_vec = open_vec[0::self.__downsampling]
    ...
    def NoiseLevelBeforeFilter(self):
        ...
        print('customNoise mean = ', np.mean(self.__customNoise), ' std = ', np.std(self.__customNoise))
        print('customNoise of Open level mean = ', np.mean(self.__customNoise_Open), ' std = ', np.std(self.__customNoise_Open))
        print()
    ...
    def GenerateHistogram(self):
        ...
        print('add custom noise ', len(self.__customNoise), len(self.__y_sos[0::self.__downsampling]))
        print('noiseFac = ', self.__noiseFac)
        noiseFac = self.__noiseFac
        summe = self.__y_sos[0::self.__downsampling] + self.__noiseFac*self.__customNoise
        [self.__hist_customNoise, self.__binEdges_customNoise]=np.histogram(summe, density=True, bins=binning) 
        
        # open level cutom noise
        open_vec = self.__open_vec
        closed_vec = -1*(self.__open_vec - np.ones(len(self.__open_vec)))
        customNoise=self.__customNoise
        self.__customTraj_Open = self.__y_sos[0::self.__downsampling]+open_vec*self.__customNoise_Open+closed_vec*customNoise 
        [self.__hist_customNoise_Open, self.__binEdges_customNoise_Open]=np.histogram(self.__customTraj_Open, density=True, bins=binning) 
    ...
    def PlotAmplitudeDistribution(self, log=1, limits = [[-75, 5], [1e-7, 1e0]],legend=0, exp=0, name=None, f_size=14):
        ...
        ax.plot(self.__binEdges_NoiseFilter[:-1], self.__hist_NoiseFilter, '--',  label='noise (added pre filter): $\sigma$ = %.2f' %self.__addNoise, linewidth=3)
        ...
        ax.plot(self.__binEdges_customNoise[:-1], self.__hist_customNoise,  label='custom noise')
        
        ax.plot(self.__binEdges_customNoise_Open[:-1], self.__hist_customNoise_Open,  label='custom noise for o. & c. level')
        if exp:
    ...
    @property
    def customNoise_traj(self):
        print('noise fac = ', self.__noiseFac)
        return self.__y_sos[0::self.__downsampling] + self.__noiseFac*self.__customNoise
    @property
    def customTraj_Open(self):
        return self.__customTraj_Open 
